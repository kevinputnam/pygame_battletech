...
# 01/12 - Moved Mech class and individual mech definitions to mechs.py
#       - Switched mech info to dictionary only, so multiple instances can be made with same data
#       - Added Hatchetman, Vindicator, Warhammer, and Commando
#       - Added handling for Melee special ability: melee_bonus attribute of mech is added to melee damage
#       - TODO - implement heat, critial hits, and special abilities
#       - TODO - implement damage to attacker for Charge and DFA
#       - TODO - add calculation for Charge - if max damage for Charge is same as Melee, don't Charge. (Melee doesn't cause damage to attacker)

import random
import mechs_2 as mechs
...
def resolve_damage(unit,target,range_modifier,weapon_range):
    ...
    if unit.attack == "Charge":
        ...
        damage = unit.size + unit.melee_bonus
...
def resolve_combat_round(friendlies,enemies):
    ...
    print("Enemy Units:")
    for unit in enemies:
        print("  * " + unit.long_name() + " PV: " + str(unit.get_pv()))
...
mech_1 = mechs.Mech({"handle":"Road Runner","jump_jets":True})
mech_2 = mechs.Mech({"handle":"Ostrich"})
mech_3 = mechs.Mech({"handle":"Glory Bird"})
mech_4 = mechs.Mech({"handle":"Seriously?"})
team_one = [mech_1,mech_2]
team_two = [mech_3,mech_4]
team_three = [mechs.Mech(mechs.archer),mechs.Mech(mechs.centurion)]
team_four = [mechs.Mech(mechs.shadow_hawk),mechs.Mech(mechs.victor)]
team_five = [mechs.Mech(mechs.warhammer),mechs.Mech(mechs.hatchetman)]
team_six = [mechs.Mech(mechs.archer),mechs.Mech(mechs.vindicator)]
#combat(team_one,team_two)
#combat(team_four,team_three)
combat(team_five,team_six)